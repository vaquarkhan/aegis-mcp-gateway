# Apache Pinot adapter example

Author: Viquar Khan.

Read-only stdio profile for the `pinot` adapter (taxonomy `olap`). Writes stay locked.

## Prerequisites

- JDK 17+
- Built shade jar: `mvn -q -pl mcp-gateway-dist -am package -DskipTests`
- Reachable backend at `PINOT_CONTROLLER_URL` (default `http://localhost:9000`)
...
$env:MCP_GW_ADAPTERS='pinot'
$env:PINOT_CONTROLLER_URL='http://localhost:9000'

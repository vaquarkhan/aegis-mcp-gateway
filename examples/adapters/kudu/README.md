# Apache Kudu adapter example

Author: Viquar Khan.

Read-only stdio profile for the `kudu` adapter (taxonomy `datastore`). Writes stay locked.

## Prerequisites

- JDK 17+
- Built shade jar: `mvn -q -pl mcp-gateway-dist -am package -DskipTests`
- Reachable backend at `KUDU_REST_URL` (default `http://localhost:8050`)

REST facade (native Kudu RPC is not bundled).
...
export MCP_GW_ADAPTERS=kudu
export KUDU_REST_URL=http://localhost:8050
...
$env:MCP_GW_ADAPTERS='kudu'
$env:KUDU_REST_URL='http://localhost:8050'

# Apache CouchDB adapter example

Author: Viquar Khan.

Read-only stdio profile for the `couchdb` adapter (taxonomy `datastore`). Writes stay locked.

## Prerequisites

- JDK 17+
- Built shade jar: `mvn -q -pl mcp-gateway-dist -am package -DskipTests`
- Reachable backend at `COUCHDB_URL` (default `http://localhost:5984`)
...
export MCP_GW_ADAPTERS=couchdb
...
Call `list_dbs` from your MCP client after connect. Mutate and destructive tools stay hidden while writes are locked.
